...
Ray-tracing method & Audio area mapping & $\times$ & $\times$ & $\bigcirc$ \\
Image source method & Early part detailed & Early part only & $\bigcirc$ & $\times$ \\
RTC method & Full detailed calculation & Full & $\bigtriangleup^{*}$ & $\bigcirc$ \\ \Hline
\multicolumn{5}{l}{$^{*}$ : have limited spatial information}
\end{tabular}%
}
\end{table}
