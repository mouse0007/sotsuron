...
\\
\\
　一方、壁面隅角部における音の散乱(端部散乱)を考慮し、付加乱反射率を与える研究$^{\text{\cite{02-7}}}$が進められている。
壁面端部から$\lambda$/8分の面積を1、それ以外の面積を0として面積平均した乱反射率を吸音面の接合面に与えたときに実測との対応が良くなることが既往の研究$^{\text{\cite{02-8}}}$から明らかになっているが、これは本研究で使用する幾何音響解析ソフトの付加乱反射率(壁面端部から$\lambda$/4分の面積を0.5、それ以外の面積を0として面積平均したもの(\textgt{式}\ref{eq:付加乱反射率計算式}、\textgt{図}\ref{fig:壁面端部}))に近似する。
...
\begin{itemize}
    ...
    $s_{\scalebox{0.5}{edge}} = 0.5$\\
    $s_{\scalebox{0.5}{surface}}$ : The normal surface scattering coefficient\\
    $S_{\scalebox{0.5}{edge}}$ : The area of an edge quarter of a wavelength wide\\
    $S_{\scalebox{0.5}{surface}}$ : The complete plane or sub-plane area\\
\end{itemize}
