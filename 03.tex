\chapter{評価手法}
\section{一元配置分散分析}
観測高さ間における室内音響物理指標や音響物理量の違いをみるため、一元配置分散分析を用いる。これは、要因が異なる複数の標本の母平均が等しい(複数の標本から求められた平均値が、同じ母平均を持つ母集団から抽出された)という仮説を検定することにより、要因の違いにより複数の標本の母平均に違いがあるか(つまり要因の違いに意味があるか)を検討するものである$^{\text{\cite{kanno}}}^{\text{\cite{池田1}}}^{\text{\cite{池田2}}}^{\text{\cite{池田3}}}$。
